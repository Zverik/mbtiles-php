...
if( isset($_GET['tileset']) ) {
	...
	try {
		...
		if( !isset($row) || $row === FALSE ) {
			...
			$format = isset($resultrow) && $resultrow !== FALSE ? $resultrow['value'] : 'png';
			if( $format == 'jpg' )
				$format = 'jpeg';
...
?>			<TileMap title="<?=$params['name'] ?>" srs="OSGEO:41001" profile="global-mercator" href="<?=$basetms.'1.0.0/'.str_replace('.mbtiles','',$file) ?>" />
<?				} catch( PDOException $e ) {}
			}
		}
	}
?>
			...
			<?
	} elseif( $_GET['tmsinfo'] == 'resource' && isset($_GET['tmslayer']) ) {
		$layer = $_GET['tmslayer'];
		try{
			...
			?><TileMap version="1.0.0" tilemapservice="<?=$basetms ?>1.0.0/">
				...
				<Abstract><?=$params['description'] ?></Abstract>
				<SRS>OSGEO:41001</SRS>
		...
		while( ($file = readdir($handle)) !== false ) {
			if( preg_match('/^[\w\d_-]+\.mbtiles$/', $file) && is_file($file) ) {
				try {
					...
					$zooms = readzooms($db);
					$db = null;
					print "<h3>".$params['name']."</h3>";
					if( isset($params['description']) )
						print "<p>".$params['description']."</p>";
					...
					print '<p>OpenLayers: <tt>new OpenLayers.Layer.OSM("'.$params['name'].'", "'.getbaseurl().preg_replace('/\.mbtiles/','',$file).'/${z}/${x}/${y}", {numZoomLevels: '.(end($zooms)+1).', isBaseLayer: '.($params['type']=='baselayer'?'true':'false').'});</tt></p>';
...
function readparams( $db ) {
	$params = array();
	foreach( $db->query("select name, value from metadata") as $row ) {
...
function readzooms( $db ) {
	$zooms = array();
	foreach( $db->query('select zoom_level from tiles group by zoom_level order by zoom_level') as $row ) {
		$zooms[] = $row['zoom_level'];
	}
	return $zooms;
}
?>
